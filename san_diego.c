...
static gboolean on_map_click(GtkWidget * widget, GdkEventButton * event, gpointer user_data) {
  ...
  if (event -> type == GDK_2BUTTON_PRESS && (left_button || right_button)) {
    ...
    if (left_button) {
        ...
        find_closest_point(lat, lon, "/home/malek/Desktop/TD_INFO/IN104/UCSD-Graphs-master/data/maps/san_diego.map", lat_result, lon_result);
...
main(int argc, char ** argv) {
  ...
  g_signal_connect(
    ...
    G_CALLBACK(on_star_align_changed), (gpointer)
    "y-align");
  Graph * graph = createGraph(3515);
  int list[20];
  int void_pos = 0;
  int Path[500] ;
  
  
  file_plot("/home/malek/Desktop/TD_INFO/IN104/UCSD-Graphs-master/data/maps/san_diego.map", map, graph);
